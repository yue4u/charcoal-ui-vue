...
async function getIcon(name: string) {
  if (!Object.hasOwn(charcoalIconFiles, name)) {
    throw new TypeError(`${name} is not a valid icon name.`);
  }
  return charcoalIconFiles[name as KnownIconFile]();
}
...
export class PixivIcon extends HTMLElement {
  ...
  svg {
    ...
    // eslint-not-disable-next-line @typescript-eslint/no-non-null-assertion
